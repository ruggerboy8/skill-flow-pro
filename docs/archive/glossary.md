> **Archived 2026-08-19 (DOC-5).** This is a historical record, accurate about the past, and is not a description of how the system works today. Do not treat it as current. For the present state see docs/README.md. Specifically wrong: it describes `user_backlog_v2` as the active store and lists `weekly_self_select` and `user_backlog` as present, but all three were dropped by migration `20260725120000`. The term definitions are still sound; the table rows are not.
