> **Archived 2026-08-19 (DOC-5).** This is a historical record, accurate about the past, and is not a description of how the system works today. Do not treat it as current. For the present state see docs/README.md. Specifically wrong: its header claims it is updated every session and should be read at the start of every new session, but it is five-plus months stale and prescribes the retired Lovable publish workflow rather than the current branch and PR kit.

# Skill Flow Pro — Progress Tracker
*No longer true, and left here struck through so the claim is visible rather than
silently removed: ~~Updated at the end of every session. Read this at the start of
every new session.~~ Last actually updated 2026-03-06. Do not read this at the
start of a session; see the archive note above.*
...
- Created `docs/enterprise-architecture.md` and `docs/archive/roadmap.md`
