...
| 2.4 | **Cycle-era retirement — Slices A-C ✅ DONE 2026-07-25, D staged.** **A** (`735c83c4`): rollover retired end-to-end (ThisWeekPanel call, `v2/rollover.ts`, `sequencer-rollover` edge fn deleted from prod) + 7 orphaned plan-era admin surfaces deleted (SequencerTestConsole, GlobalPlanManager, TimeTravelPanel, ProposedWeekEditor, PlanHistory, LockedWeekViewer, SimpleProMovePicker — all zero-importer). **B** (`703b0527`): `weekly_scores.site_action_id` backfilled for all eras (5,658/5,660) making score history self-describing; sequencer-rank simplified + redeployed; wizard weekly_plan/focus fallbacks replaced with weekly_assignments; submission hook assign:-only (also removed its call to the dropped backlog RPC); Builder MonthView now reads weekly_assignments (**fixes its scheduled markers, blank since the weekly_plan freeze**); Review page + hook + `unifiedAssignments.md` deleted. **C** (`f1b538ab`): the 3 live history RPCs (win banner, My Role history, staff-detail history) rewritten to resolve via `site_action_id` — **fixes the participant win banner, empty for the whole assignments era**; verified live (real win returned; year-long 158-row history intact). 19 dead cycle-era DB functions classified. **D — ✅ APPLIED 2026-07-24** after the publish + Kelly wizard test passed (which also caught and hotfixed a slice-B dangling ref, `0c969c19`): tables archived as `zzz_archived_*`, 19 dead functions + both `get_staff_week_assignments` overloads dropped, `types.ts` regenerated, last caller (`weekAssembly` legacy "progress mode" sim branch) removed. Verified post-drop: coach view, win banner, history all healthy. **2.4 COMPLETE.** Remaining residue tracked separately: coach_scope column drop (needs admin-users get/update cleanup), legacy flag-write retirement (needs RPC eligibility modernization). | The cycle era is archived |
