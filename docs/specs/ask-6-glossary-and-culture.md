...
### Alcan seed (17 entries, authored 2026-08-25)

> Correction 2026-08-26: three entries removed after review. "Membership
> plan" was a `general_uk` concept (Alcan has none). "Thinkific" (retired
> LMS) and "Reach" (after-hours vendor) were unverified assumptions with no
> corpus source; the on-call/after-hours system is TBD pending a
> conversation with Dr. Alex. Seed went 20 → 17 entries; no `assumption`
> entries remain. The provenance grounding block now restricts pro-move
> citations to `practice_types @> ['pediatric_us']`, so a UK doc can never
> back a glossary entry. See the ASK-5 spec's practice-type correction for
> the matching mirror fix.
