-- SEC-2 batch A verification: AFTER state.
-- Read-only. Run against the live database after applying
-- supabase/migrations/20260819232211_sec2a_revoke_anon_execute_leaking_functions.sql
-- to confirm anon lost EXECUTE on every targeted function and authenticated
-- kept it.
--
-- Expect every row's anon_execute to read false here, and every row's
-- authenticated_execute to still read true.
