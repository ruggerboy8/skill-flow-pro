...
function suggestEmployee(staff: Participant, roster: DeputyEmployee[]): DeputyEmployee | null {
  if (staff.email) {
    ...
    const byEmail = roster.find((d) => d.email && d.email.toLowerCase() === lower);
    if (byEmail) return byEmail;
  }
  const norm = normalizeName(staff.name);
  const exact = roster.find((d) => normalizeName(d.display_name) === norm);
  if (exact) return exact;
  // Token-overlap fallback
  const sa = new Set(norm.split(/\s+/).filter(Boolean));
  let best: { d: DeputyEmployee; score: number } | null = null;
  for (const d of roster) {
    const sb = new Set(normalizeName(d.display_name).split(/\s+/).filter(Boolean));
    let inter = 0;
  ...
  return best && best.score >= 0.6 ? best.d : null;
}
