...
// ─── Helpers ────────────────────────────────────────────────────────────────
// Name-matching helpers live in src/lib/deputyMatching.ts and are imported
// above so the wizard, the dashboard's mapping panel, and the invite dialog
// all use the same suggestion logic.
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            When on, the dispatcher runs every Monday at 3:00 AM Central and applies excusals for the
            previous week (Mon–Sun). Existing excusals are skipped, so re-runs are safe.
          </p>
        </CardHeader>
        <CardContent className="space-y-3">
      ...
      <SyncHistoryPanel organizationId={organizationId} />
    </div>
  );
}

function SyncHistoryPanel({ organizationId }: { organizationId: string }) {
  const { data: runs, isLoading } = useQuery({
    queryKey: ["deputy-sync-runs", organizationId],
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from("deputy_sync_runs")
        .select("id, mode, trigger, week_start, week_end, status, excusals_inserted, excusals_already_existed, mapped_participant_count, error_message, started_at, finished_at")
        .eq("organization_id", organizationId)
