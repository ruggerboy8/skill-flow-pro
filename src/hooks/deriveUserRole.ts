import type { StaffProfile } from './useStaffProfile';

/**
 * Pure derivation of role/permission flags from a staff profile.
 *
 * Extracted from useUserRole so it can be unit tested without mocking
 * Supabase or React Query. No behavior change from the inline version —
 * this is the same logic, verbatim, just callable on its own.
 */
export function deriveUserRole(staff: StaffProfile) {
