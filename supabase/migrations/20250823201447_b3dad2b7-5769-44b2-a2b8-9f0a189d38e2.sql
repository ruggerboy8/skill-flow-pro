-- Fix replace_weekly_focus function to remove non-existent iso_year and iso_week columns
CREATE OR REPLACE FUNCTION public.replace_weekly_focus(p_cycle integer, p_week_in_cycle integer, p_role_id bigint, p_slots jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Insert new slots (removed iso_year and iso_week columns)
