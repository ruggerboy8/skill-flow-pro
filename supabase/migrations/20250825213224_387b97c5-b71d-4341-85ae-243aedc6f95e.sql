...
BEGIN
  ...
  FOR i IN REVERSE (p_weeks - 1)..0 LOOP
    ...
    -- Check confidence and performance scores for this week
    SELECT 
      CASE 
    ...
    week_data := jsonb_build_object(
      ...
      'conf_ts', null,  -- Simplified for now
      'perf_ts', null   -- Simplified for now
    );
...
BEGIN
  ...
  LOOP
    ...
    -- Get last p_window completed weeks with performance data in this domain
    FOR week_record IN
      SELECT DISTINCT 
        wf.cycle,
        wf.week_in_cycle,
...
BEGIN
  ...
  FOR domain_record IN 
    ...
    ORDER BY d.domain_name
  LOOP
    -- Calculate means for this domain across recent weeks
    SELECT 
    ...
    WHERE ws.staff_id = p_staff_id
      ...
      AND ws.performance_score IS NOT NULL
      AND wf.cycle >= (SELECT MAX(cycle) - 1 FROM weekly_focus WHERE role_id = p_role_id); -- Last ~2 cycles
