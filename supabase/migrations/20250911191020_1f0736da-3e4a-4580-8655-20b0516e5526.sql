-- Fix get_strengths_weaknesses to return domain_id and domain_name for competency rows
...
BEGIN
  ...
  competency_results AS (
    ...
    GROUP BY b.competency_id, c.name, b.domain_id, b.domain_name
  )
  SELECT dr.level, dr.id, dr.name, dr.n_items, dr.avg_observer, dr.domain_id, dr.domain_name FROM domain_results dr
  UNION ALL
  SELECT cr.level, cr.id, cr.name, cr.n_items, cr.avg_observer, cr.domain_id, cr.domain_name FROM competency_results cr
