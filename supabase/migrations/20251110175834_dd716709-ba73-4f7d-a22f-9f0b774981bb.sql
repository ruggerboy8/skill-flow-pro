-- Fix get_calendar_week_status to deduplicate by week_of
...
BEGIN
  ...
  WITH onboarding_weeks AS (
    SELECT
      ...
      SUM(CASE WHEN ws.weekly_focus_id IS NOT NULL THEN 1 ELSE 0 END) AS total,
      COUNT(ws.confidence_score) AS conf_count,
      COUNT(ws.performance_score) AS perf_count,
      MIN(wf.cycle) AS cycle,
      MIN(wf.week_in_cycle) AS week_in_cycle,
