...
BEGIN
  -- Check if coach is super admin
  SELECT s.is_super_admin INTO is_super_admin
  FROM staff s
  WHERE s.user_id = p_coach_user_id
  LIMIT 1;

  -- Get coach's org (for non-super-admins)
  ...
  WITH visible_staff AS (
    ...
    WHERE s.is_participant = true
      AND (COALESCE(is_super_admin, false) OR coach_org_id IS NULL OR l.organization_id = coach_org_id)
  ),
  cycle_calc AS (
    SELECT
      vs.*,
  ...
  scores AS (
    SELECT
      ...
      (SELECT GREATEST(MAX(ws.confidence_date), MAX(ws.performance_date))
       FROM weekly_scores ws
       WHERE ws.staff_id = e.staff_id
         AND ws.weekly_focus_id = ANY(e.required_ids)) AS last_activity_at
    FROM enriched e
  ),
  backlog AS (
    SELECT
  ...
  states AS (
    SELECT
      ...
      CASE
        WHEN a.conf_count < a.required_count THEN 'confidence'
        WHEN a.perf_count < a.required_count THEN 'performance'
        ELSE NULL
  ...
  SELECT
    ...
    CASE states.status_state_calc
      ...
      WHEN 'can_checkin' THEN 'Check-in Available'
      WHEN 'missed_checkin' THEN 'Missed Check-in'
      WHEN 'wait_for_thu' THEN 'Waiting for Thursday'
      WHEN 'can_checkout' THEN 'Check-out Available'
      WHEN 'missed_checkout' THEN 'Missed Check-out'
    ...
    CASE states.status_state_calc
      ...
      WHEN 'can_checkin' THEN 'Ready to submit confidence scores'
      WHEN 'missed_checkin' THEN 'Check-in deadline has passed'
      WHEN 'wait_for_thu' THEN 'Waiting for performance window'
      WHEN 'can_checkout' THEN 'Ready to submit performance scores'
      WHEN 'missed_checkout' THEN 'Check-out deadline has passed'
      WHEN 'done' THEN 'All scores submitted'
    ...
    states.last_activity_at,
    states.last_activity_kind_calc AS last_activity_kind,
    CASE
      WHEN states.last_activity_at IS NOT NULL THEN 
        to_char(states.last_activity_at AT TIME ZONE states.timezone, 'Mon DD, YYYY')
      ELSE 'No activity'
...
-- Set search path and permissions
...
CREATE INDEX IF NOT EXISTS idx_weekly_scores_staff_focus ON weekly_scores(staff_id, weekly_focus_id);
CREATE INDEX IF NOT EXISTS idx_weekly_plan_lookup ON weekly_plan(role_id, week_start_date, org_id) WHERE status = 'locked';
CREATE INDEX IF NOT EXISTS idx_weekly_focus_lookup ON weekly_focus(role_id, cycle, week_in_cycle);
CREATE INDEX IF NOT EXISTS idx_user_backlog_open ON user_backlog_v2(staff_id) WHERE resolved_on IS NULL;