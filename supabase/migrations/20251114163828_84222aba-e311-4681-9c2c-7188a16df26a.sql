-- Complete rewrite of get_staff_statuses with canonical week calculation and proper type handling
...
BEGIN
  ...
  week_adjusted AS (
    SELECT
      wc.*,
      -- If before Friday 5pm, use previous week's Monday
      CASE 
        WHEN p_now < wc.fri_17_utc 
        THEN (wc.monday_local - interval '1 week')::date
        ELSE wc.monday_local
      END AS assignment_monday,
      p_now < wc.fri_17_utc AS before_deadline
    FROM week_calc wc
  ),
  week_indexed AS (
    SELECT
