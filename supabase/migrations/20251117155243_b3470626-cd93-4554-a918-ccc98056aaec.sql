...
-- Fix get_staff_statuses to properly match scores to the correct week
-- The issue is that scores are being counted based on cycle/week numbers
-- without verifying they were submitted during the correct time window.
...
anchors AS (
  SELECT
    ...
    -- Calculate Monday and Sunday boundaries for this week in UTC
    (cc.active_monday || ' 00:00:00 ' || cc.tz)::timestamptz AS week_start_utc,
    ((cc.active_monday + 6) || ' 23:59:59 ' || cc.tz)::timestamptz AS week_end_utc
  FROM cycle_calc cc
),
source_choice AS (
  SELECT
    ...
    CASE
      WHEN a.source_used = 'plan' THEN (
        ...
        WHERE ws.staff_id = a.staff_id
          ...
          AND ws.confidence_date >= a.week_start_utc
          AND ws.confidence_date <= a.week_end_utc
          AND EXISTS (
      ...
      ELSE (
        ...
        WHERE ws.staff_id = a.staff_id
          ...
          AND EXISTS (
            ...
            WHERE wf.id = ws.weekly_focus_id::uuid
    ...
    CASE
      WHEN a.source_used = 'plan' THEN (
        ...
        WHERE ws.staff_id = a.staff_id
          ...
          AND ws.performance_date >= a.week_start_utc
          AND ws.performance_date <= a.week_end_utc
          AND EXISTS (
            SELECT 1 FROM weekly_plan wp
            WHERE wp.id = (substring(ws.weekly_focus_id from 'plan:(\d+)')::bigint)
              AND wp.role_id = a.role_id::int
...
-- Grant permissions
ALTER FUNCTION get_staff_statuses(uuid, timestamptz) SET search_path = public;
GRANT EXECUTE ON FUNCTION get_staff_statuses(uuid, timestamptz) TO authenticated;
