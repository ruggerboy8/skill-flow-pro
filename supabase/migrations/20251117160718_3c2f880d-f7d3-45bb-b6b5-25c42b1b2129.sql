...
-- Fix off-by-one error in cycle/week calculation
-- Week 18 with 6-week cycles should be Cycle 3, Week 6, not Cycle 4, Week 1
...
cycle_calc AS (
  SELECT
    wc.*,
    -- Fix: subtract 1 from week_index before division to handle 1-indexed weeks
    CASE 
  ...
  SELECT
    ...
    CASE
      ...
      ELSE (
        SELECT COUNT(*)::int
        FROM weekly_scores ws
        JOIN weekly_focus wf ON wf.id = ws.weekly_focus_id::uuid
        WHERE ws.staff_id = a.staff_id
          AND ws.confidence_score IS NOT NULL
