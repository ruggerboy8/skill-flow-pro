...
-- Fix get_staff_statuses to use week_of column properly
...
scores AS (
  SELECT
    a.*,
    -- Use week_of column for reliable week detection
    (
      SELECT COUNT(*)::int
      FROM weekly_scores ws
      WHERE ws.staff_id = a.staff_id
        AND ws.week_of = a.active_monday
...
COMMENT ON FUNCTION get_staff_statuses IS 'Returns staff status using week_of column for reliable week detection';
