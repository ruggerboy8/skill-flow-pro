-- Rebuild view_staff_submission_windows to remove cartesian duplication, honor hire_date start, and align status semantics
...
staff_participation AS (
  SELECT
    sl.*,
    (date_trunc('week', sl.hire_date + interval '1 day')::date + interval '7 days')::date AS participation_start_monday
