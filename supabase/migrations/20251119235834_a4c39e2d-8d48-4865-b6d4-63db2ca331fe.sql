-- Fix delete_week_data_by_week function to handle UUID/text type mismatch
...
BEGIN
  ...
  -- Step 3: Delete weekly_self_select using the captured list (FIX: cast UUID to text)
  DELETE FROM weekly_self_select
  WHERE user_id = v_user_id
    AND weekly_focus_id::text = ANY(focus_ids_to_delete);
