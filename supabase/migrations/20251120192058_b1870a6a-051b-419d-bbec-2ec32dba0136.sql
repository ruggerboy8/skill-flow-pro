-- Fix the get_best_weekly_win function with renamed return columns to avoid ambiguity
...
BEGIN
  ...
  week_scores AS (
    ...
    LEFT JOIN pro_moves pm2 ON pm2.action_id = wp.action_id
    ...
    WHERE ws.confidence_score IS NOT NULL
      AND ws.performance_score IS NOT NULL
      AND (pm.action_statement IS NOT NULL OR pm2.action_statement IS NOT NULL)
  ),
  perfect_weeks AS (
