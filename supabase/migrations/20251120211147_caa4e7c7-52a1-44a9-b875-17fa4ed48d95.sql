-- Add participation_start_at column to staff table
ALTER TABLE public.staff 
ADD COLUMN IF NOT EXISTS participation_start_at timestamptz;

COMMENT ON COLUMN public.staff.participation_start_at IS 'The date when this staff member began participating in the pro-moves program. Used to filter out pre-participation weeks from their history.';

-- Set Kelly's participation start to Sept 16, 2024 (1 week after hire for onboarding)
UPDATE public.staff 
SET participation_start_at = '2024-09-16 00:00:00+00'::timestamptz
WHERE email = 'kelly.acuna@steinerranch.alcan.dental';

-- Update get_calendar_week_status to respect participation_start_at
...
BEGIN
  ...
  SELECT 
    ...
    l.timezone
  INTO v_location_id, v_participation_start, v_program_start, v_cycle_length, v_tz
  FROM staff s
  ...
  -- Use hire_date as fallback if participation_start_at is null
  IF v_participation_start IS NULL THEN
    SELECT hire_date::timestamptz INTO v_participation_start
  ...
  WHERE aw.week_of >= date_trunc('week', v_participation_start)::date
