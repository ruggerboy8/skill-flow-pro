...
-- Revert get_calendar_week_status to NOT filter by participation_start_at
-- This allows historical scores to appear in accordion even if before participation_start_at
DROP FUNCTION IF EXISTS public.get_calendar_week_status(uuid, bigint) CASCADE;
DROP FUNCTION IF EXISTS public.get_calendar_week_status(uuid, integer) CASCADE;

CREATE OR REPLACE FUNCTION public.get_calendar_week_status(p_staff_id uuid, p_role_id integer)
RETURNS TABLE(
  cycle integer,
