-- Fix on-time rate calculation to recognize historical submissions in the same ISO week
-- Problem: Kelly's historical scores have week_of='2024-09-29' but system expects '2024-09-30'
-- Solution: Match by ISO week rather than exact Monday date
...
score_data AS (
  ...
  LEFT JOIN weekly_scores ws ON
    ...
    -- Flexible week matching: same ISO week (Monday-Sunday)
    AND date_trunc('week', ws.week_of::timestamp)::date = date_trunc('week', ca.week_of::timestamp)::date
    AND (
