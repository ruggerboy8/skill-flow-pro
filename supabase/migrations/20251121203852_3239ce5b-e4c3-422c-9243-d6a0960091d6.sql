-- Fix view_staff_submission_windows to support weekly_assignments (V2) with proper source priority
-- Priority: weekly_assignments > weekly_plan > weekly_focus
...
-- V2: weekly_assignments (highest priority)
week_assignment_data AS (
  SELECT
    wc.staff_id,
    wc.staff_name,
    wc.role_id,
    ...
    wa.display_order,
    wa.self_select,
    ('assign:' || wa.id::text) AS assignment_id
  FROM week_context wc
  JOIN weekly_assignments wa ON
    wa.week_start_date = wc.week_of
    AND wa.role_id = wc.role_id
    AND (wa.location_id = wc.location_id OR wa.location_id IS NULL AND wa.org_id = wc.org_id OR wa.org_id IS NULL AND wa.location_id IS NULL)
    AND wa.status = 'active'
    AND wa.action_id IS NOT NULL
),
-- Legacy: weekly_plan (medium priority)
week_plan_data AS (
  SELECT
    wc.staff_id,
    wc.staff_name,
  ...
  WHERE NOT EXISTS (
    ...
    WHERE wa.week_start_date = wc.week_of
      AND wa.role_id = wc.role_id
      AND (wa.location_id = wc.location_id OR wa.location_id IS NULL AND wa.org_id = wc.org_id OR wa.org_id IS NULL AND wa.location_id IS NULL)
      AND wa.status = 'active'
  )
),
-- Legacy: weekly_focus (lowest priority)
week_focus_data AS (
  SELECT
    wc.staff_id,
    wc.staff_name,
...
combined_assignments AS (
  SELECT * FROM week_assignment_data
  UNION ALL
...
latest_scores AS (
  SELECT * FROM (
    SELECT
      ws.*, 
      COALESCE(ws.assignment_id, ws.weekly_focus_id, 'site:' || ws.site_action_id::text) AS assignment_key,
      row_number() OVER (
        PARTITION BY ws.staff_id, ws.week_of, COALESCE(ws.assignment_id, ws.weekly_focus_id, 'site:' || ws.site_action_id::text)
        ORDER BY COALESCE(ws.performance_date, ws.confidence_date, ws.updated_at, ws.created_at) DESC
      ) AS rn
    FROM weekly_scores ws
    WHERE ws.assignment_id IS NOT NULL OR ws.weekly_focus_id IS NOT NULL OR ws.site_action_id IS NOT NULL
  ) ranked
  WHERE rn = 1
),
score_data AS (
