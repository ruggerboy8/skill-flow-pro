...
-- Fix get_staff_statuses to prioritize weekly_assignments over weekly_focus
-- This prevents double-counting when both exist for the same week
...
BEGIN
  ...
  -- CHANGED: Only use weekly_assignments OR weekly_focus, not both
  -- If weekly_assignments exist for this week/role/location, use them
  -- Otherwise fall back to weekly_focus
  has_assignments AS (
    SELECT DISTINCT
      s.id as staff_id,
      s.role_id,
  ...
  assignments_cte AS (
    -- Get from weekly_assignments for locations that have them
    SELECT 
      ha.staff_id,
      ha.source,
    ...
    -- Get from weekly_focus for locations that don't have weekly_assignments
    SELECT 
      ha.staff_id,
      ha.source,
  ...
  scores AS (
    ...
    LEFT JOIN weekly_scores ws ON ws.staff_id = s.id
      -- Match scores to assignments by week_of
