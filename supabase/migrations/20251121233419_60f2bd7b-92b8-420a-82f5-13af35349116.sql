-- Fix: Join scores by week_of, not by assignment week_start_date
...
BEGIN
  ...
  -- Get assignments for the selected week to count required
  assignments_cte AS (
    SELECT
      wi.staff_id,
  ...
  -- Get scores by week_of (not by assignment reference)
  scores_cte AS (
    SELECT
      ws.staff_id,
      ws.confidence_score,
  ...
  SELECT
    ...
    COALESCE(COUNT(DISTINCT CASE WHEN sc.confidence_score IS NOT NULL AND sc.week_of = wi.week_start THEN sc.staff_id END), 0)::int as conf_submitted_count,
    COALESCE(COUNT(DISTINCT CASE WHEN sc.confidence_late = true AND sc.week_of = wi.week_start THEN sc.staff_id END), 0)::int as conf_late_count,
    COALESCE(COUNT(DISTINCT CASE WHEN sc.performance_score IS NOT NULL AND sc.week_of = wi.week_start THEN sc.staff_id END), 0)::int as perf_submitted_count,
    COALESCE(COUNT(DISTINCT CASE WHEN sc.performance_late = true AND sc.week_of = wi.week_start THEN sc.staff_id END), 0)::int as perf_late_count,
