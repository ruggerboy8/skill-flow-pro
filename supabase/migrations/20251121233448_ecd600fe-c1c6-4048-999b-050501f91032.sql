-- Fix: Count actual score records, not just distinct staff
...
BEGIN
  ...
  -- Count scores for the selected week
  week_scores AS (
    SELECT
      ws.staff_id,
      COUNT(*) FILTER (WHERE ws.confidence_score IS NOT NULL) as conf_count,
      COUNT(*) FILTER (WHERE ws.confidence_late = true) as conf_late,
      COUNT(*) FILTER (WHERE ws.performance_score IS NOT NULL) as perf_count,
      COUNT(*) FILTER (WHERE ws.performance_late = true) as perf_late
    FROM weekly_scores ws
    JOIN week_info wi ON wi.staff_id = ws.staff_id AND ws.week_of = wi.week_start
    GROUP BY ws.staff_id
  ),
  -- Count required assignments for the week
  required_counts AS (
    SELECT
      wi.staff_id,
      COUNT(*) as required
    FROM week_info wi
    JOIN roster r ON r.staff_id = wi.staff_id
    JOIN weekly_assignments wa
      ON wa.week_start_date = wi.week_start
      AND wa.role_id = r.role_id
      AND wa.status = 'locked'
      AND (
        wa.location_id = r.location_id
        OR (wa.location_id IS NULL AND wa.org_id = r.organization_id)
        OR (wa.org_id IS NULL AND wa.location_id IS NULL)
      )
    GROUP BY wi.staff_id
  ),
  ...
  SELECT
    ...
    COALESCE(rc.required, 0)::int as required_count,
    COALESCE(ws.conf_count, 0)::int as conf_submitted_count,
    COALESCE(ws.conf_late, 0)::int as conf_late_count,
    COALESCE(ws.perf_count, 0)::int as perf_submitted_count,
    COALESCE(ws.perf_late, 0)::int as perf_late_count,
  ...
  LEFT JOIN week_scores ws ON ws.staff_id = r.staff_id
  LEFT JOIN required_counts rc ON rc.staff_id = r.staff_id
