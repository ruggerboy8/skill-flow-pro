-- Drop and recreate get_staff_statuses to use Central Time for week calculations
DROP FUNCTION IF EXISTS public.get_staff_statuses(uuid, date);

CREATE FUNCTION public.get_staff_statuses(
...
DECLARE
  v_week_start date;
BEGIN
  -- Determine week start: use p_week_start if provided, otherwise compute Monday in Central Time
  IF p_week_start IS NOT NULL THEN
    v_week_start := p_week_start;
  ELSE
    -- Get current date in Central Time and roll back to Monday
  ...
  visible_staff AS (
    ...
    WHERE s.is_participant
      ...
      AND (
        ...
        OR (cs.coach_scope_type IS NULL) -- super admin sees all
      )
  ),
  staff_assignments AS (
    ...
    LEFT JOIN weekly_assignments wa ON 
      ...
      AND (
        wa.org_id IS NULL 
        OR wa.org_id = (SELECT l.organization_id FROM locations l WHERE l.id = st.primary_location_id)
      )
      AND (
        wa.location_id IS NULL
        OR wa.location_id = st.primary_location_id
      )
  ),
  staff_scores AS (
    SELECT
  ...
  staff_aggregates AS (
    SELECT
      ss.staff_id,
      COUNT(sa.assignment_id) AS required_count,
    ...
    INNER JOIN staff_assignments sa ON sa.staff_id = ss.staff_id AND sa.assignment_id = ss.assignment_id
