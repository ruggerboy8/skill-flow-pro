-- Drop unused get_staff_statuses version (single parameter)
DROP FUNCTION IF EXISTS public.get_staff_statuses(p_week_start date);

-- Recreate get_staff_statuses with FIXED date normalization
-- ACTIVE VERSION: This is the one called by useCoachStaffStatuses hook
...
DECLARE
  v_week_start date;
BEGIN
  -- FIX: Normalize to Monday for both provided and default dates
