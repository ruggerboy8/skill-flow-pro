-- Phase 1: get_staff_weekly_scores RPC
-- Returns raw weekly_scores data for all participant staff visible to a coach
...
BEGIN
  ...
  IF v_coach_scope_type IS NULL AND NOT EXISTS (
    SELECT 1 FROM staff WHERE user_id = p_coach_user_id AND is_super_admin
  ) THEN
    RETURN;
  END IF;
  ...
  WHERE s.is_participant = true
    ...
    AND (
      v_coach_scope_type IS NULL -- super admin
