-- Update get_staff_weekly_scores to:
-- 1. Return only the most recent week's data (6 rows per staff max)
-- 2. Fix week_of to display Monday instead of Sunday

CREATE OR REPLACE FUNCTION public.get_staff_weekly_scores(p_coach_user_id uuid)
RETURNS TABLE(
  staff_id uuid,
  staff_name text,
  staff_email text,
...
BEGIN
  ...
  -- Get the most recent week_of date from any staff member's scores
  SELECT MAX(date_trunc('week', ws.week_of)::date)
  ...
  SELECT
    ...
    ws.id AS score_id,
    date_trunc('week', ws.week_of)::date AS week_of, -- Convert to Monday
    ws.assignment_id,
    wa.action_id,
  ...
  LEFT JOIN weekly_scores ws ON ws.staff_id = fs.id
    AND date_trunc('week', ws.week_of)::date = v_most_recent_week -- Only most recent week
