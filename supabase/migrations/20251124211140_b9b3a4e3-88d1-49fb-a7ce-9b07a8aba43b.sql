-- Fix week_of to properly display Monday's date instead of Sunday
...
BEGIN
  ...
  -- Get the most recent week_of date, converted to Monday
  ...
  SELECT
    ...
    ws.id AS score_id,
    (ws.week_of::date - ((EXTRACT(DOW FROM ws.week_of)::int + 6) % 7))::date AS week_of, -- Convert to Monday
    ws.assignment_id,
    wa.action_id,
  ...
  LEFT JOIN weekly_scores ws ON ws.staff_id = fs.id
    AND (ws.week_of::date - ((EXTRACT(DOW FROM ws.week_of)::int + 6) % 7))::date = v_most_recent_week -- Only most recent week
