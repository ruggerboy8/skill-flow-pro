-- Drop the old version with date parameter to resolve ambiguity
-- Keep only the text version since the frontend passes string dates
DROP FUNCTION IF EXISTS public.get_staff_weekly_scores(uuid, date);

-- Ensure we have the correct text version (recreate to be safe)
