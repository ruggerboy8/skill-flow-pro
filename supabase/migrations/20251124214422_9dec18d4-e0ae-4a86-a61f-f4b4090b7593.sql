-- Fix get_staff_weekly_scores to correctly join weekly_self_select
-- weekly_self_select uses weekly_focus_id, so join via legacy_focus_id
...
DECLARE
  v_week_start date;
BEGIN
  v_week_start := date_trunc('week', COALESCE(p_week_of::date, (NOW() AT TIME ZONE 'America/Chicago')::date))::date;
  ...
  SELECT
    ...
    afw.action_id,
    COALESCE(wss.selected_pro_move_id, afw.action_id) AS selected_action_id,
    ws.confidence_score,
    ws.confidence_date,
    CASE 
  ...
  LEFT JOIN weekly_self_select wss ON 
    wss.weekly_focus_id = afw.legacy_focus_id
    AND wss.user_id = sd.user_id
