-- Fix ambiguous column references in get_staff_weekly_scores
...
BEGIN
  ...
  assignment_with_focus_bridge AS (
    ...
    LEFT JOIN weekly_self_select wss
      ON wss.user_id = (SELECT s2.user_id FROM staff s2 WHERE s2.id = afw.staff_id)
      AND wss.weekly_focus_id IN (
        SELECT wf.id::text
        FROM weekly_focus wf
        WHERE wf.role_id = (SELECT s3.role_id FROM staff s3 WHERE s3.id = afw.staff_id)
