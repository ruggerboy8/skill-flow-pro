-- Fix type mismatch in get_staff_all_weekly_scores
-- assignment_id in weekly_scores is text, but weekly_assignments.id is uuid
...
BEGIN
  ...
  assignment_scores AS (
    ...
    LEFT JOIN weekly_scores ws ON 
      ...
      AND ws.assignment_id = ('assign:' || wa.id::text)  -- Fixed: cast UUID to text and prefix
  ),
  focus_scores AS (
    SELECT 
      si.id AS staff_id,
