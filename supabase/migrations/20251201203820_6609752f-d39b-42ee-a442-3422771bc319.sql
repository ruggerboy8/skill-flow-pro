-- Fix get_staff_all_weekly_scores to respect onboarding vs graduated status
-- The RPC should use weekly_focus for cycles 1-3 (onboarding) and weekly_assignments for cycle 4+ (graduated)
...
BEGIN
  ...
  week_cycles AS (
    ...
    FROM staff_info si
    CROSS JOIN (
      SELECT DISTINCT week_start_date, cycle 
      FROM weekly_focus 
      WHERE role_id = (SELECT role_id FROM staff_info)
    ) wf
