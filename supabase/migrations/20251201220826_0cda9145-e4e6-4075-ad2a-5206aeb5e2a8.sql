-- Phase 1.3: Re-label today's onboarding scores to use assignment_id (FIXED v2)
-- Fix scores submitted today (2025-12-01) by onboarding staff
...
FROM (
  ...
  WHERE ws_inner.assignment_id IS NULL
    ...
    AND EXISTS (
      -- Only for onboarding cycles (C1-C3)
