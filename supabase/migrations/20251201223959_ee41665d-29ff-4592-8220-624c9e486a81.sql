-- Drop and recreate get_staff_weekly_scores with onboarding/global filtering
...
BEGIN
  ...
  applicable_assignments AS (
    ...
    WHERE wa.role_id = sd.role_id
      AND wa.week_start_date = v_target_week_start
    ...
    WHERE wf.role_id = sd.role_id
      AND wf.week_start_date = v_target_week_start
  ),
  scores_data AS (
    SELECT
      aa.staff_id,
