-- Fix get_staff_weekly_scores: replace c.action_id with scores.selected_action_id
...
BEGIN
  ...
  FROM staff s
  WHERE s.user_id = p_coach_user_id AND s.is_coach = true
  LIMIT 1;
  ...
  filtered_assignments AS (
    ...
    WHERE NOT EXISTS (
      ...
      WHERE loc.onboarding_active = true
        AND loc.organization_id = v_org_id
