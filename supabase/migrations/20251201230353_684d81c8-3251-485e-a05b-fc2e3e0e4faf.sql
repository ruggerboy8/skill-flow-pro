-- Surgical fix: Add super admin support and onboarding/global filtering to get_staff_weekly_scores
-- Based on working Nov 25 version with minimal targeted changes
...
CREATE FUNCTION public.get_staff_weekly_scores(p_coach_user_id uuid, p_week_of text DEFAULT NULL::text)
 RETURNS TABLE(staff_id uuid, staff_name text, staff_email text, user_id uuid, role_id bigint, role_name text, location_id uuid, location_name text, organization_id uuid, organization_name text, score_id uuid, week_of date, assignment_id text, action_id bigint, selected_action_id bigint, confidence_score integer, confidence_date timestamp with time zone, confidence_late boolean, confidence_source text, performance_score integer, performance_date timestamp with time zone, performance_late boolean, performance_source text, action_statement text, domain_id bigint, domain_name text, display_order integer, self_select boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INTO v_is_authorized, v_is_super_admin, v_coach_scope_type, v_coach_scope_id
  FROM staff s
  ...
  SELECT
    ...
    ('assign:' || es.assignment_id)::text AS assignment_id,
    es.action_id,
