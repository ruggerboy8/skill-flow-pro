...
-- Fix the get_staff_weekly_scores RPC to use 'org' instead of 'organization'
-- and also check the coach_scopes table for multi-scope users
...
BEGIN
  ...
  WHERE s.user_id = p_coach_user_id
    AND (s.is_coach OR s.is_super_admin OR s.is_org_admin)
  LIMIT 1;
  ...
  IF p_week_of IS NOT NULL THEN
    v_most_recent_week := date_trunc('week', p_week_of)::date;
  ELSE
