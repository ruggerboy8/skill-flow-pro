-- Drop any existing versions of the function to avoid ambiguity
DROP FUNCTION IF EXISTS public.get_location_domain_staff_averages(uuid, uuid[], integer[], text[], timestamptz, timestamptz, boolean);
DROP FUNCTION IF EXISTS public.get_location_domain_staff_averages(uuid, timestamptz, timestamptz, boolean, uuid[], integer[], text[]);
...
BEGIN
  ...
  staff_domain_agg AS (
    ...
    LEFT JOIN eval_items ei ON ei.staff_id = s.id AND ei.domain_id = d.id
    WHERE l.organization_id = p_org_id
      AND s.active = true
      AND (p_location_ids IS NULL OR s.location_id = ANY(p_location_ids))
      AND (p_role_ids IS NULL OR s.role_id = ANY(p_role_ids))
    GROUP BY s.id, s.full_name, s.role_id, r.name, s.location_id, l.name, d.id, d.name, ei.eval_status
  )
  SELECT
    sda.location_id,
    sda.location_name,
