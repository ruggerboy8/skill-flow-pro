-- Fix view_staff_submission_windows to include the on_time column
-- The get_staff_submission_windows function expects this column
...
WITH base_staff AS (
  ...
  WHERE s.is_participant = true
    AND s.is_paused = false  -- Exclude paused users
),
assignment_weeks AS (
...
conf_data AS (
  ...
  LEFT JOIN weekly_scores ws ON ws.staff_id = ad.staff_id AND ws.assignment_id = 'assign:' || ad.assignment_id
),
perf_data AS (
  SELECT 
    ad.staff_id,
    ...
    (ad.week_of + INTERVAL '6 days' + INTERVAL '12 hours') AT TIME ZONE ad.timezone AS due_at
  FROM assignments_data ad
  LEFT JOIN weekly_scores ws ON ws.staff_id = ad.staff_id AND ws.assignment_id = 'assign:' || ad.assignment_id
)
SELECT 
  ...
  CASE 
    ...
    WHEN now() AT TIME ZONE 'America/Chicago' > cd.due_at THEN 'missing'
    ELSE 'pending'
  END AS status,
  cd.submitted_at,
  cd.submitted_late,
  cd.due_at,
  -- Add on_time column: true if submitted and not late
  CASE 
...
SELECT 
  ...
  CASE 
    ...
    WHEN now() AT TIME ZONE 'America/Chicago' > pd.due_at THEN 'missing'
    ELSE 'pending'
  END AS status,
  pd.submitted_at,
  pd.submitted_late,
