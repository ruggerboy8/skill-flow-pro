...
-- Recreate the view with a more flexible score matching strategy
-- that handles cross-location-linked legacy scores
...
WITH base_staff AS (
  ...
  LEFT JOIN weekly_scores ws ON (
    ...
    AND (
      -- Primary match: direct assignment_id link
      ws.assignment_id = ('assign:' || ad.assignment_id)
      -- Fallback: cross-location-linked scores (same action, same week, different location's assignment)
      OR (
...
SELECT cd.staff_id, cd.staff_name, cd.role_id, cd.location_id,
  cd.week_of, cd.cycle_number, cd.week_in_cycle, cd.action_id,
  cd.is_self_select, cd.slot_index, cd.required,
  ...
  CASE
    ...
    ELSE 'pending'
  END AS status,
  cd.submitted_at, cd.submitted_late, cd.due_at,
  CASE
...
SELECT pd.staff_id, pd.staff_name, pd.role_id, pd.location_id,
  pd.week_of, pd.cycle_number, pd.week_in_cycle, pd.action_id,
  pd.is_self_select, pd.slot_index, pd.required,
  ...
  CASE
    ...
    ELSE 'pending'
  END AS status,
  pd.submitted_at, pd.submitted_late, pd.due_at,
  CASE
