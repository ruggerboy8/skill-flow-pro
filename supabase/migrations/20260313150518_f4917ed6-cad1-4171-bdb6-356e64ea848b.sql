-- Fix view_staff_submission_windows: org_id references organizations.id, not practice_groups.id
-- The join condition must go through practice_groups.organization_id
...
WITH base_staff AS (
  ...
  SELECT cc.staff_id, cc.staff_name, cc.role_id, cc.location_id,
    cc.week_of, cc.cycle_number, cc.week_in_cycle,
    cc.timezone, cc.conf_due_day, cc.conf_due_time, cc.perf_due_day, cc.perf_due_time,
    wa.id AS assignment_id, wa.action_id,
  ...
  JOIN weekly_assignments wa ON (
    ...
    AND (
      ...
      -- Org-level assignment: org_id is organizations.id, resolve through practice_groups
      OR (wa.org_id IS NOT NULL AND wa.location_id IS NULL AND EXISTS (
        SELECT 1 FROM locations l2
        JOIN practice_groups pg ON pg.id = l2.group_id
        WHERE l2.id = cc.location_id AND pg.organization_id = wa.org_id
      ))
      -- Global assignment (legacy)
      OR (wa.org_id IS NULL AND wa.location_id IS NULL)
    )
  )
), conf_data AS (
  SELECT ad.staff_id, ad.staff_name, ad.role_id, ad.location_id,
    ad.week_of, ad.cycle_number, ad.week_in_cycle,
    ad.action_id, ad.is_self_select, ad.slot_index, ad.required,
    ad.timezone,
    ws.confidence_score,
    ws.confidence_date AS submitted_at,
    ws.confidence_late AS submitted_late,
    ((ad.week_of + (ad.conf_due_day || ' days')::interval + ad.conf_due_time::interval) AT TIME ZONE ad.timezone) AS due_at
  FROM assignments_data ad
  LEFT JOIN weekly_scores ws ON (
    ws.staff_id = ad.staff_id
    AND (
      ws.assignment_id = 'assign:' || ad.assignment_id
      OR (
        ws.week_of = ad.week_of
        AND ws.assignment_id IS NOT NULL
        AND ws.assignment_id <> 'assign:' || ad.assignment_id
        AND EXISTS (
          SELECT 1 FROM weekly_assignments wa2
          WHERE ws.assignment_id = 'assign:' || wa2.id::text
          AND wa2.action_id = ad.action_id
        )
      )
    )
  )
  ...
  SELECT ad.staff_id, ad.staff_name, ad.role_id, ad.location_id,
    ...
    ((ad.week_of + (ad.perf_due_day || ' days')::interval + ad.perf_due_time::interval) AT TIME ZONE ad.timezone) AS due_at
  FROM assignments_data ad
  LEFT JOIN weekly_scores ws ON (
    ws.staff_id = ad.staff_id
    AND (
...
SELECT cd.staff_id, cd.staff_name, cd.role_id, cd.location_id,
  cd.week_of, cd.cycle_number, cd.week_in_cycle,
  cd.action_id, cd.is_self_select, cd.slot_index, cd.required,
...
UNION ALL
SELECT pd.staff_id, pd.staff_name, pd.role_id, pd.location_id,
  pd.week_of, pd.cycle_number, pd.week_in_cycle,
  pd.action_id, pd.is_self_select, pd.slot_index, pd.required,
