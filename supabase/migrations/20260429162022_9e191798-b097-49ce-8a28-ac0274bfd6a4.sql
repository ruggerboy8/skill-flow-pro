-- Lower self-score threshold from n>=2 to n>=1 in compute_eval_self_scores.
-- UI will visually flag n=1 as low confidence so coaches know to weigh it carefully.
...
BEGIN
  ...
  SET
    ...
    self_score = CASE WHEN a.n >= 1 THEN ROUND(a.avg_score)::int ELSE NULL END,
    self_is_na = false,
    self_note = NULL
  FROM agg a
...
-- Recompute all existing draft Quarterly evals with the new threshold
DO $$
DECLARE r record;
BEGIN
  FOR r IN SELECT id FROM public.evaluations WHERE type='Quarterly' AND status='draft' AND quarter IS NOT NULL
  LOOP
