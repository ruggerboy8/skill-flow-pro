-- Phase 2 P0: review_payload v3 carries per-competency Glow/Grow.
--
-- Additive and conservative: bumping the cached version from 2 to 3 means any
-- existing v2 payload is recomputed on next view; the only new content is
-- observer_glow / observer_grow on each candidate item. All ranking, sparse,
-- and domain-summary logic is unchanged from v2. Applied to the live project
-- via MCP; kept here for repo parity (apply via the dashboard SQL Editor if
-- replaying). Idempotent (CREATE OR REPLACE).
...
BEGIN
  ...
  -- Version cache: return if already v3
  IF v_eval.review_payload IS NOT NULL
     AND (v_eval.review_payload->>'version')::int = 3 THEN
