-- Fix get_staff_all_weekly_scores: org-scoped locked assignments (location_id
-- NULL, org_id set) never surfaced for staff without a score row, because the
-- unscored_assignments CTE compared weekly_assignments.org_id (which holds an
-- organizations.id) against staff → locations.group_id (a practice_groups.id).
-- Those never match. Resolve org_id against the staff's organization id
-- (practice_groups.organization_id). Also tolerate the practice_groups.id form
-- for safety, though no rows currently use it.
