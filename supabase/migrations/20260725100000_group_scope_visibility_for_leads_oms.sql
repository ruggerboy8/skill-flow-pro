...
--
-- 1) get_staff_weekly_scores: previously, ANY caller with can_view_submissions
--    (i.e. every lead) was granted ORGANIZATION-wide visibility via
--    v_has_org_team_access. Remove can_view_submissions from that grant so leads
--    fall through to their coach_scopes rows. can_view_submissions REMAINS in the
--    caller-eligibility check (leads can still call the RPC).
--    NOTE ON NAMING: coach_scopes.scope_type = 'org' actually references
--    practice_groups.id (legacy naming — "org" was the old term for group).
--
-- 2) Additive scope backfill: every active lead/OM gets a group scope row for
--    their primary location's practice group (if missing). Existing rows are
--    kept — the OM helper functions (get_office_manager_location_id,
--    is_office_manager_for_location) require scope_type='location' rows.
...
BEGIN
  SELECT
    ...
    (
      ...
      -- can_view_submissions deliberately NOT here (2026-07-25): it marks
      -- "may see submissions within their scope", not org-wide visibility.
...
-- Part 2: additive group-scope backfill for active leads and OMs.
insert into public.coach_scopes (id, staff_id, scope_type, scope_id)
select gen_random_uuid(), s.id, 'org', l.group_id
from public.staff s
join public.locations l on l.id = s.primary_location_id
where (s.is_lead = true or s.is_office_manager = true)
  and s.is_paused is not true
  and l.group_id is not null
  and not exists (
    select 1 from public.coach_scopes cs
    where cs.staff_id = s.id and cs.scope_type = 'org' and cs.scope_id = l.group_id
  );

-- Sanity: every active lead/OM with a location must now have a group scope row.
do $$
...
begin
  ...
  if v_missing > 0 then
    raise exception 'Group-scope backfill incomplete: % leads/OMs missing group scope', v_missing;
  end if;
end $$;
