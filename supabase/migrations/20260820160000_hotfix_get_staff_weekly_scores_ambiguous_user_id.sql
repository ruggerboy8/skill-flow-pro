-- HOTFIX applied to production 2026-08-20 during a live outage.
--
-- SEC-2b added an authorization guard to get_staff_weekly_scores:
--   SELECT 1 FROM public.staff WHERE user_id = auth.uid() AND is_super_admin = true
-- The column `user_id` there was UNQUALIFIED. Because this function
-- RETURNS TABLE(..., user_id uuid, ...), that output column name is a
-- variable in the PL/pgSQL scope, so `user_id` in the guard was ambiguous
-- between the OUT column and staff.user_id. Every call threw
-- "column reference \"user_id\" is ambiguous", breaking the coach dashboard
-- and the regional command center (both call this function via
-- useStaffWeeklyScores).
--
-- Fix: qualify the guard's column references (s.user_id, s.is_super_admin).
-- Nothing else in the function changed. Idempotent (CREATE OR REPLACE).
--
-- Follow-up (separate ticket): the same unqualified guard exists in the other
-- SEC-2b functions; they do not currently throw only because none of them has
-- a colliding OUT column. They should be qualified defensively, and SQL
-- function changes need execution testing (this class of bug is invisible to
-- static review).
